...
for exact `--icon-ids`, read-only `--status`, `--dispatch --enable-inline`,
model price floors, checkpoints, cumulative budgets and bounded recovery. Inline
work suppresses global wakeups; use `--resume-inline` for abandoned inline work.
Dry-run is read-only but may read
