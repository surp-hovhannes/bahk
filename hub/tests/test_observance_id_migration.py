...
class ObservanceKeyMigrationTests(TransactionTestCase):
    ...
    migrate_from = ("hub", "0064_fastintention_matched_tags_and_more")
    migrate_data_to = ("hub", "0066_backfill_feast_observance_ids")
    migrate_to = ("hub", "0067_finalize_feast_observance_id")
    ...
    def _seed(self):
        ...
        current = Feast.objects.create(church=church, name=CURRENT)
        FeastContext.objects.create(
    ...
    def test_both_spellings_collapse_onto_one_observance(self):
        ...
        keyed = Feast.objects.filter(church_id=church.id, observance_id=KEY)
    ...
    def test_every_context_is_reparented_and_the_feedback_total_is_kept(self):
        ...
        survivor = Feast.objects.get(church_id=church.id, observance_id=KEY)
    ...
    def test_several_unresolved_rows_can_coexist_under_the_new_constraint(self):
        ...
        self.assertEqual(
            Feast.objects.filter(church_id=church.id, observance_id__isnull=True).count(), 2)
    ...
    def test_each_church_is_re_keyed_on_its_own(self):
        ...
        self.assertEqual(Feast.objects.get(church_id=other_church.id).observance_id, KEY)
        # Same observance, two churches: the constraint is per church, so both survive.
        self.assertEqual(Feast.objects.filter(observance_id=KEY).count(), 2)
    ...
    def test_running_forward_twice_changes_nothing(self):
        ...
        self.assertEqual(
            Feast.objects.filter(church_id=church.id, observance_id=KEY).count(), 1)
    ...
    def test_the_name_constraint_is_gone_before_the_data_migration_runs(self):
        ...
        Feast.objects.create(church_id=church.id, name="Fast day", observance_id="fast_day")
        Feast.objects.create(
            church_id=church.id, name="Fast day", observance_id="illuminator_fast_day_3")
