...
The rename is not cosmetic: ``Feast`` is keyed by ``(church, observance_id)``; the name used to
be the lookup key, so getting it wrong either strands two years of LLM contexts and curated icons
under a name nothing reaches, or merges two commemorations that are not the same feast.  Both
fail silently, which is why the merge path is exercised against real rows here rather than
trusted to read correctly.  The fold that lets a mangled scrape name match clean engine text is
pinned in both directions, over the whole artifact rather than a sample, for the same reason.
...
Grouped by what each block protects:
  ...
  * ``ObservanceIdTests`` -- the identity a row is keyed by, and why it is not the name.
...
class NameMapTests(TestCase):
    """The shipped artifact, and that it still agrees with the engine actually installed.
