...
class FeastModelTests(TestCase):
    ...
    def test_the_same_observance_twice_in_one_church_is_rejected(self):
        ...
        Feast.objects.create(church=self.church, name="Epiphany", observance_id="epiphany")
        with self.assertRaises(IntegrityError):
            Feast.objects.create(church=self.church, name="Theophany",
                                 observance_id="epiphany")
    ...
    def test_the_same_name_under_two_observances_is_allowed(self):
        ...
        "Fast day" in English.
        """
        Feast.objects.create(church=self.church, name="Fast day", observance_id="fast_day")
        Feast.objects.create(church=self.church, name="Fast day",
                             observance_id="illuminator_fast_day_3")

        self.assertEqual(Feast.objects.filter(name="Fast day").count(), 2)

    def test_rows_with_no_observance_id_do_not_collide(self):
        ...
        self.assertEqual(
            Feast.objects.filter(church=self.church, observance_id__isnull=True).count(), 2)
