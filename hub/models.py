...
class Feast(models.Model):
    ...
    church = models.ForeignKey(
        ...
        related_name="feasts",
        default=Church.get_default_pk,
        help_text="The church this feast belongs to",
    )
    name = models.CharField(
        max_length=256,
        help_text="The name of the feast",
    )
    ...
    class Meta:
        constraints = [
            constraints.UniqueConstraint(
                fields=["date", "church"],
                name="unique_feast_per_date_church",
            ),
        ]
        indexes = [
            models.Index(fields=["date", "church"]),
            models.Index(fields=["date"]),
        ]
        ordering = ["date"]
...
class LLMPrompt(models.Model):
    ...
    applies_to = models.CharField(
        ...
        default="readings",
        help_text="What model this prompt applies to (readings or feasts)",
    )
    ...
    def save(self, *args, **kwargs):
        """Override save to ensure only one prompt can be active at a time per applies_to type."""
        if self.active:
            # Check if there are any other active prompts for the same applies_to type excluding the current one
            ...
            if other_active_prompts.exists():
                raise ValidationError(
                    f"Another LLM prompt for {self.get_applies_to_display()} is already marked as active. Please deactivate it first."
                )
        super().save(*args, **kwargs)
    ...
    def __str__(self):
        ...
        return f"{self.model} prompt for {applies_to_display}: {self.prompt[:20]}{status}"
...
class FeastContext(models.Model):
    """Model for storing context for feasts, typically generated by an LLM."""
    ...
    short_text = models.TextField(
        help_text="A shorter version of the context text",
        null=True,
        blank=True,
    )
    prompt = models.ForeignKey(
