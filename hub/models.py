...
class LLMPrompt(models.Model):
    ...
    model_type = models.CharField(
        ...
        default="readings",
        help_text="The type of model this prompt applies to (readings or feasts)",
    )
    ...
    def save(self, *args, **kwargs):
        """Override save to ensure only one prompt can be active at a time per model_type."""
        if self.active:
            # Check if there are any other active prompts of the same model_type excluding the current one
            other_active_prompts = LLMPrompt.objects.filter(
                active=True, model_type=self.model_type
            ).exclude(pk=self.pk)
            if other_active_prompts.exists():
                raise ValidationError(
                    f"Another LLM prompt for {self.get_model_type_display()} is already marked as active. Please deactivate it first."
                )
        super().save(*args, **kwargs)
