...
``scripts/build_feast_name_map.py`` against the engine ``requirements.txt`` pins, currently 1.3.0.
Each entry records an old spelling, the name it resolved to **and the date it resolved on**.  The
date is the durable half, and the gap between the two halves is measured rather than asserted:
rebuilding this same artifact against 2.1.0 leaves every one of the 244 entries in place and
changes 138 of their target NAMES, against 3 of their dates.
