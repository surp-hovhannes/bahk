...
``Feast`` is keyed by ``(church, observance_id)`` -- one of the engine's published ids, so a row
is one commemoration.  A published id keeps meaning the same observance, so unlike the name it does not move
...
from hub.services.feast_rename import (
    apply_group, describe, engine_names, load_name_map, observance_ids, plan_renames,
