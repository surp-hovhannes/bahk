"""Generates seed data for feasts with python manage.py import_feasts."""
...
from hub.utils import scrape_feasts
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for date_obj in daterange(start_date, end_date):
            feast_data = scrape_feasts(date_obj, church)
            if feast_data:
                ...
                feast_obj, created = models.Feast.objects.get_or_create(
                    date=date_obj,
                    church=church,
                    defaults={"name": name_en}  # This becomes name_en in the database
                )
                ...
                if name_hy and not feast_obj.name_hy:
                    ...
                    logging.info(f"Created feast: {feast_obj}")
