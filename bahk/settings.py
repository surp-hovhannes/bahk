...
if not (config('CI', default=False, cast=bool) or 'test' in sys.argv):
    sentry_sdk.init(
        ...
        integrations=[
            ...
            RedisIntegration(),
            # Add logging integration to capture logs as Sentry logs
            LoggingIntegration(
...
INSTALLED_APPS = [
    'bahk.admin_config.FastAndPrayAdminConfig',
    'django.contrib.auth',
...
if config('DATABASE_URL', default=None):
    # Production and staging use the database provided through DATABASE_URL.
    DATABASES = {
...
# Configure available languages for modeltrans
...
READING_CONTEXT_REGENERATION_THRESHOLD = config('READING_CONTEXT_REGENERATION_THRESHOLD', default=1, cast=int)
FEAST_CONTEXT_REGENERATION_THRESHOLD = config('FEAST_CONTEXT_REGENERATION_THRESHOLD', default=1, cast=int)
...
# Per consumer/church overrides may be supplied in deployment settings.
